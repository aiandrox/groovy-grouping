  ...
  describe '.group' do
    ...
    context '条件があるとき' do
      ...
      it '条件で均等に分ける' do
        ...
        expect(group_1_attendance_statuses.pluck(:criterion_status_id).uniq).to include(criterion_status_1.id, criterion_status_2.id)
        expect(group_2_attendance_statuses.pluck(:criterion_status_id).uniq).to include(criterion_status_1.id, criterion_status_2.id)
      end
    end
  end
end
